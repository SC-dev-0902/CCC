...
import { SignInCard } from "@/components/auth-card"
...
export default function Page() {
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-sm font-medium">Claude Command Center</span>
        <button
          onClick={toggle}
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-10">
        <SignInCard theme={theme} />
      </div>
      <footer
        className="px-5 py-3 text-center text-[11px] shrink-0"
        style={{ borderTop: `1px solid ${t.border}`, color: t.textMuted }}
      >
        First time here? <Link href="/setup" style={{ color: t.accent }}>Run first-run setup</Link>
        <span style={{ marginLeft: 12 }}>or</span>
        <Link href="/" style={{ color: t.accent, marginLeft: 6 }}>Back to dashboard</Link>
      </footer>
    </div>
  )
}
