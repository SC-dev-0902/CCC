...
> **CC test run (2026-05-05):** All CLI/API items executed end-to-end on Dev-Web by CC and ticked. Browser/treeview UI items (Task 2 visual confirmation, project registration walkthrough) left unchecked for Phet to walk. Note: `pkill -f "node server.js"` from an SSH bash shell can match the parent shell's own command line and kill the session - use `pgrep -x node` + per-PID `/proc/$p/cmdline` filter instead. CC restarted via a one-shot `/tmp/ccc-restart.sh` launcher; current PID 6475, build 62.
>
> **Proxy housekeeping (commit `143c4e8`):** mid-test, the canonical CCC URL switched from direct port `http://172.16.10.6:3000/` to the Apache-proxied form `http://kkh01vdweb01.mcsfam.local/CCC/`. New Apache conf `/etc/apache2/conf-available/CCC.conf` (Dev-Web only, not in repo) reverse-proxies `/CCC/` and `/CCC/ws` onto `127.0.0.1:3000`, and excludes `/CCC/design-preview` so the Stage 01f static alias is preserved. Frontend now derives a `BASE_PATH` from `window.location.pathname` so `api()` and the xterm WebSocket work under either mount. CLI re-ticks below were re-verified against the proxied URL after the change; both forms remain functional.
...
> Test comment: PASS. Restart used a one-shot `/tmp/ccc-restart.sh` launcher (kills `node server.js` by PID via `/proc/$p/cmdline` to avoid the pkill self-kill described above). Log: `CCC running on http://localhost:3000`. PID 6475.

- [x] Confirm port 3000 responds.
  - Command: `curl -s -o /dev/null -w "%{http_code}\n" http://kkh01vdweb01.mcsfam.local/CCC/api/version`
  - Outcome: `200`.

> Test comment: PASS. `200`. `/api/version` -> `{"version":"1.0.7","build":"62"}` (build 62 = post-04b commit `e8809cd`).

---

### Scaffold the Fixture

- [x] Build a fixture project at `/tmp/04b-fixture/` matching the new v1.1 layout.
  - Command:
    ```
...
> Test comment: PASS. 18 entries listed, matching the expected tree (4 test files + concept + tasklist + 11 parent/leaf dirs).

---

### `getVersionFolder()` (Task 1a)

- [x] `getVersionFolder('1.0')` returns `v1.0` (no `docs/` prefix).
  - Command: `cd /mnt/sc-development/CCC && node -e "const v=require('./src/versions'); console.log(v.getVersionFolder('1.0'));"`
  - Outcome: `v1.0`.

> Test comment: PASS. `v1.0`.

- [x] `getVersionFolder('1.1.1')` returns `v1.1/v1.1.1`.
  - Command: `cd /mnt/sc-development/CCC && node -e "const v=require('./src/versions'); console.log(v.getVersionFolder('1.1.1'));"`
  - Outcome: `v1.1/v1.1.1`.

> Test comment: PASS. `v1.1/v1.1.1`.

---

### `scanVersions()` against fixture (Tasks 1b + 1c)

- [x] `scanVersions()` finds `v1.0` at project root, no `flatTestFiles` key, `folder` is `v1.0`.
...
> Test comment: PASS. `keys: ["versions","hasFlatDocs"]` (no `flatTestFiles`), `vCount: 1`, `folder: "v1.0"`, `version: "1.0"`, `patches: 1`.

- [x] Concept and tasklist files are picked up from `v1.0/docs/`.
...
> Test comment: PASS. `["Project_concept_v1.0.md", {"name":"Project_tasklist_v1.0.md","stagesCompleted":1,"stagesTotal":2}]`.

- [x] Test files include `stagePath` for both flat and nested cases.
...
> Test comment: PASS. All three entries present in the expected order: `stage01 (1/3)`, `stage01/stage01a (2/2)`, `stage02 (0/1)`.

- [x] Patch version `v1.0.1` nested inside `v1.0/` is detected and scans correctly.
...
> Test comment: PASS. `version: "1.0.1"`, `folder: "v1.0/v1.0.1"`, one test file with `stagePath: "stage03"` and 1/1 checked.

---

### `getTestFilePath()` (Task 1d)

- [x] Numeric stageId zero-pads, returns the correct relative path, and creates the stage subfolder.
...
> Test comment: PASS. `{"path":"v1.0/docs/testfiles/stage05/Project_test_stage05.md","dirExists":true}`.

- [x] Sub-stage stageId (`11a`) is used as-is.
...
> Test comment: PASS. `v1.0/docs/testfiles/stage11a/Project_test_stage11a.md`.

- [x] Fix sub-stage stageId (`11a01`) is used as-is.
...
> Test comment: PASS. `v1.0/docs/testfiles/stage11a01/Project_test_stage11a01.md`.

- [x] Patch version produces the nested path.
...
> Test comment: PASS. `v1.0/v1.0.1/docs/testfiles/stage07/Project_test_stage07.md`.

---

### `createVersion()` (Task 1e)

- [x] `createVersion()` scaffolds `vX.Y/docs/`, `vX.Y/docs/handoff/`, `vX.Y/docs/testfiles/`, plus seeded concept + tasklist.
  - Command:
    ```
...
> Test comment: PASS. All six expected entries present in the tree (`v1.2`, `v1.2/docs`, `v1.2/docs/handoff`, `v1.2/docs/testfiles`, `v1.2/docs/Demo_concept_v1.2.md`, `v1.2/docs/Demo_tasklist_v1.2.md`).

- [x] Patch type creates the nested layout `vX.Y/vX.Y.Z/docs/...`.
  - Command:
    ```
    cd /mnt/sc-development/CCC && \
...
> Test comment: PASS. Six expected entries under `v1.2/v1.2.1`, including seeded `Demo_concept_v1.2.1.md` and `Demo_tasklist_v1.2.1.md` inside `docs/`.

---

### `migrateToVersioned()` (Task 1f)

- [x] Moves `docs/{name}_concept.md` and `docs/{name}_tasklist.md` into `vX.Y/docs/`, plus creates `handoff/` and `testfiles/`.
  - Command:
    ```
    cd /mnt/sc-development/CCC && \
...
> Test comment: PASS. Return: `{"folder":"v1.0","version":"1.0","moved":["Demo_concept.md","Demo_tasklist.md"]}`. Tree confirmed: docs files now under `v1.0/docs/`, `handoff/` and `testfiles/` created, original `docs/Demo_*.md` files no longer listed (only the empty `docs/` dir remains as expected).

---

### `server.js` callers (Task 3)

- [x] `GET /api/projects/:id/test-file-path?stage=99` returns the new path pattern AND the stage subfolder is auto-created on disk.
  - Pre-step (find a registered project id, e.g. CCC itself):
    `curl -s http://kkh01vdweb01.mcsfam.local/CCC/api/projects | node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const o=JSON.parse(s);const p=o.projects.find(x=>x.name==='CCC')||o.projects[0];console.log('id:',p.id,'name:',p.name,'activeVersion:',p.activeVersion);});"`
  - Command: `curl -s "http://kkh01vdweb01.mcsfam.local/CCC/api/projects/<ID>/test-file-path?stage=99" | python3 -m json.tool` (substitute `<ID>` from the pre-step).
...
> Test comment: PASS. CCC's id `3b7c2ac6-…3f8`, activeVersion `1.1`. Response: `{"path":"v1.1/docs/testfiles/stage99/CCC_test_stage99.md","absPath":"/mnt/sc-development/CCC/v1.1/docs/testfiles/stage99/CCC_test_stage99.md"}`. Disk verified: directory exists. Side-effect cleanup: `/mnt/sc-development/CCC/v1.1/` was removed after this test (it was created purely by `getTestFilePath`'s mkdirSync) so the live CCC project tree returns to its pre-04b state. Stage 08's Migration Wizard will land the real `v1.1/` directory.

- [x] No regressions in the import-scan endpoint (it still calls `scanVersions`).
  - Command: `curl -s -X POST -H 'Content-Type: application/json' -d '{"dirPath":"/mnt/sc-development/CCC"}' http://kkh01vdweb01.mcsfam.local/CCC/api/scan-project | python3 -c "import sys,json; o=json.load(sys.stdin); print('valid:',o.get('valid'),'| versioning keys:',sorted(o.get('versioning',{}).keys()))"`
...
> Test comment: PASS (after URL fix - the route is `POST /api/scan-project` with body `{dirPath}`, not the GET shape originally written here). Response: `valid: True`, `versioning keys: ['hasFlatDocs','hasVersionedDocs','suggestedActiveVersion']`, `hasVersionedDocs: False`, `hasFlatDocs: False`. CCC's pre-04b legacy `docs/v1.0/` and `docs/v1.1/` paths are correctly invisible to the new scanner.

---

### Treeview Test File Grouping (Task 2)

- [ ] Open `http://kkh01vdweb01.mcsfam.local/CCC/` in the browser. Cmd+Shift+R to bypass cache.
...
- [ ] Cleanup: remove the throwaway `04b-fixture` project (keeps the registry clean).
  - Command: substitute the id from the registration response: `curl -s -X DELETE http://kkh01vdweb01.mcsfam.local/CCC/api/projects/<ID>`
...
> Test comment: PARTIAL by CC - `/tmp/04b-create`, `/tmp/04b-patch`, `/tmp/04b-migrate` already removed by CC after the CLI run. `/tmp/04b-fixture` was re-scaffolded so Phet can walk the UI items above. After the UI walk, run `rm -rf /tmp/04b-fixture` to finish.

---

### Acceptance Criteria

- [x] AC1 - `scanVersions()` scans version folders at project root (`vX.Y/`), not inside `docs/`.
- [x] AC2 - `scanVersionFiles()` returns concept/tasklist files from `vX.Y/docs/` and test files from `vX.Y/docs/testfiles/` with correct `stagePath` values.
- [x] AC3 - Patch versions (`vX.Y.Z/`) nested inside `vX.Y/` are correctly detected and scanned.
- [x] AC4 - `getTestFilePath()` returns the new path pattern and creates the stage subfolder if it does not exist.
- [x] AC5 - `createVersion()` creates `vX.Y/docs/`, `vX.Y/docs/handoff/`, and `vX.Y/docs/testfiles/` with seeded concept + tasklist files.
- [x] AC6 - `migrateToVersioned()` moves flat docs into `vX.Y/docs/` (not `docs/vX.Y/`).
- [x] AC7 - All `getTestFilePath()` callers in `server.js` pass `projectAbsPath` as first argument. (Verified: `grep -n getTestFilePath server.js` shows the sole call site at line 612 passing `found.absPath` first.)
- [ ] AC8 - Treeview Testing section groups test files by stage with correct hierarchy. (Browser-side - left for Phet.)
- [x] AC9 - `flatTestFiles` is no longer present in the `scanVersions()` result.
- [x] AC10 - No references to `docs/vX.Y` remain in `src/versions.js` after the change. (Verified: `grep -c "docs/v" src/versions.js` returns 0.)
