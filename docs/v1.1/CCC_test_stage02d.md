...
- [ ] Open the design preview. Locate the sidebar header.
...
- [ ] Type `lead` in the filter (lowercase).
...
- [ ] Clear the input, then type `LEAD` in upper case.
...
- [ ] Clear the input, then type `nexus-admin`.
...
- [ ] Clear the search. Confirm **Nexus** is collapsed by default (no chevron-down, no sub-projects visible).
  - Outcome: Nexus row shows a right-pointing chevron and only the parent name + status dot. No sub-projects rendered.

> Test comment:

- [ ] Type `nexus-core` in the filter.
...
- [ ] Clear the search and confirm **LeadSieve** stays expanded (it expands by default).
...
- [ ] Clear the search. Confirm the **NEW** group header is visible at the top of the tree, with `analytics-service` listed under it (italic, `unregistered` badge).
...
- [ ] Type a string that matches no project in any group, e.g. `zzzzzz`.
...
- [ ] With `zzzzzz` still in the field, inspect the **PARKED** group.
...
- [ ] Type `lead` to filter Active down to LeadSieve.
  - Outcome: filtered tree visible.

> Test comment:

- [ ] With the cursor still in the input, press **Escape**.
...
- [ ] Type `nexus-core` (auto-expands Nexus). Press **Escape**.
...
- [ ] Toggle to Dark theme. Repeat the `lead`, `web`, and `zzzzzz` queries.
...
- [ ] Toggle to Light theme. Repeat the same three queries.
  - Outcome: same behaviour, legible in light theme.

> Test comment:

- [ ] Open the browser DevTools console and reload the page. Type a filter, clear it, press Escape.
